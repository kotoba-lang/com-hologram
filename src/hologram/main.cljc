(ns hologram.main
  "Kotodama WASM entrypoint for the Hologram clean-room actor (L4) — Clojure port.
  ...
  facts (`hologram.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hologram")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hologram.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hologram-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hologram-compat/ready)
